...
const ACTIONS = {
  openModal: 'rnmu:open-modal',
  closeModal: 'rnmu:close-modal',
  addOutlet: 'rnmu:add-modal',
  removeOutlet: 'rnmu:remove-outlet',
} as const;

const reducer = (
  state: {
    outlets: RNMUContextState['outlets'];
  },
  action: {
    type: string;
...
export const useRNMUState = () => {
  const [state, dispatch] = useReducer(reducer, {
    outlets: {},
  });

  const openModal: RNMUContextState['openModal'] = ({
    modalId,
    modalComponent,
    outletId,
  }) => {
  ...
  const closeModal: RNMUContextState['closeModal'] = ({ modalId, outletId }) => {
    dispatch({ type: ACTIONS.closeModal, payload: { modalId, outletId } });
  };

  const addOutlet: RNMUContextState['addOutlet'] = outletId => {
    dispatch({ type: ACTIONS.addOutlet, payload: { outletId } });
  };

  const removeOutlet: RNMUContextState['removeOutlet'] = outletId => {
